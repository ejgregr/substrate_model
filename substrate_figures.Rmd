...
**Figure 4: Comparisons of predictions from three different models (20 m weighted – top row; 20 m unweighted – middle row, and 100 m weighted – bottom row) for the Pacific Rim (left column) and Greater Vancouver (right column) assessment areas.**
...
```{r Fig7a_Depth_Results_by_Resolution_TSS, echo=FALSE, fig.width=10, fig.height=6, out.width='100%'}


# This code  builds the data used by all three figure panels.
z <- depth.results
...
```{r Fig7b_Depth_Results_by_Resolution_TPR, echo=FALSE}
...
```{r Fig7c_Depth_Results_by_Resolution_TNR, echo=FALSE}
...
**Figure 7: : Difference in the (A) true skill statistic (TSS), (B) overall accuracy (TPR), and (C) true negative rate (TNR) between the 20 and 100 m models across depth ranges, by region, usng the build testing partition. Values below 0 indicate a higher score by the 20 m model. As these are difference plots, no corrections for the random baseline are required.**  
...
```{r Fig8a_Depth_Results_Obs_Pontius, echo=FALSE, fig.width=10, fig.height=6, out.width='100%'}
...
```{r Fig8b_Depth_Results_Obs_Pontius, echo=FALSE, fig.width=10, fig.height=6, out.width='100%'}
...
**Figure 8b: Regional performance of (A) the 100m model and (B) the 20 m models across depth zones, using the build testing partition**  
...
```{r Fig9a_IDS_By_Ribbon_A, echo=FALSE, out.width='120%', fig.fullwidth=TRUE}
...
```{r Fig9b_IDS_By_Ribbon_B, echo=FALSE, out.width='120%', fig.fullwidth=TRUE}
...
```{r Fig9c_IDS_By_Ribbon_C, echo=FALSE, out.width='120%', fig.fullwidth=TRUE}
...
**Figure 9c: Accuracy and error assessment statistics for independent data sets by depth zone for three models with sufficient sample size: Coastwide, HG, and NCC.**  
...
![](C:/Users/Edward/Dropbox/DFO Job/Substrate2019/Results/figures/ms_figure10_ScaleComp.png){ fig.fullwidth=TRUE }

**Figure 10: Mapped predictions for A) two 20 m regional models (HG and NCC) and B) the 100 m coastwide model. Note the detail provided by the 20 m models near shore where the 100 m model predicts largely Rock. In contrast, the 100 m model captures known features at depth not captured by the 20 m model.** 
...
```{r FigS1_Prevalence_IDS, echo=FALSE, out.width='120%', fig.fullwidth=TRUE}
...
```{r FigS2_Prevalence_ObsPred, echo=FALSE, fig.fullwidth=TRUE }
