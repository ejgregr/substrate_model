...
Table X (Manually built): Results from density comparison (TSS not corrected).  
Include regional models. Table or figure? 

Density	N	Imbalance	OOB	TSS	Accuracy	TNRWtd	Quantity	Exchange	Shift
Low	27282	0.123	0.295	0.565	0.684	0.871	0.046	0.257	0.013
High	38774	0.205	0.295	0.574	0.713	0.846	0.037	0.243	0.006


Eds comments:  
Results are a bit surprising. Thought the difference would be larger. Interesting that dense areas are also more out of balance, but the Quantity error is nevertheless lower.   
...
Include table for IDS results (which ones?)  
...
```{r IDS_Three, echo=FALSE, out.width='120%', fig.fullwidth=TRUE}
# Pontius stats (Accuracy, Quantity, Exchange, Shift) by ribbon in regions ... 
# Remember: Accuracy = 1 – (Quantity + Exchange + Shift).
...
**Figure 9: Pontius statistics for Independent data sets by region, by depth ribbon.**  
...
**Figure 10: Comparison of prevalence between model build(?) and study area predictions.**    
...
X-axis labels suck.  Observed / Mapped.  Points / Surface  
