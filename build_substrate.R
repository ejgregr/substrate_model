...
if ( trimrngr == T ) {
   ...
   starttime <- Sys.time()
   cat( 'Starting trimmed Ranger build: ', starttime, '\n\n' )
   ...
   #-- Pull the top 6 predictors out of the data ... 
   bigshots <- names( foo$variable.importance )[1:6]
   x <- bar[ bar$TestDat == 0, c( 'BType4', bigshots )] #train
   y <- bar[ bar$TestDat == 1, c( 'BType4', bigshots )] #test
   
      new.form <- paste0( 'BType4 ~ ', paste0( bigshots, sep = ' + ', collapse = '') )
   new.form <- substr( new.form, 0, nchar( new.form )-3 )

   #-- And rebuild/re-evaluate the model. 
   foo <- NoWt.Ranger.Model( x, y, new.form)
   imp <- foo$Model$variable.importance
   
   # Store results  ... 
   trm.region.Coast <- foo$Model
   trm.build.results <- c( trm.build.results, 'Coast' = list( 'stats' = foo$Stats, 'Import' = imp) )
   ...
   # Store results  ... 
   trm.region.HG <- foo$Model
   trm.build.results <- c( trm.build.results, 'HG' = list( 'stats' = foo$Stats, 'Import' = imp) )
   ...
   # Store results  ... 
   trm.region.NCC <- foo$Model
   trm.build.results <- c( trm.build.results, 'NCC' = list( 'stats' = foo$Stats, 'Import' = imp) )
   ...
   # Store results  ... 
   trm.region.WCVI <- foo$Model
   trm.build.results <- c( trm.build.results, 'WCVI' = list( 'stats' = foo$Stats, 'Import' = imp) )
   ...
   # Store results  ... 
   trm.region.QCS <- foo$Model
   trm.build.results <- c( trm.build.results, 'QCS' = list( 'stats' = foo$Stats, 'Import' = imp) )
   ...
   # Store results  ... 
   trm.region.SOG <- foo$Model
   trm.build.results <- c( trm.build.results, 'SOG' = list( 'stats' = foo$Stats, 'Import' = imp) )
   
   
   #----------------------------------
   #-- Done building trimmed wtd ranger models. 
   ...
   save( trm.region.Coast, trm.region.HG, trm.region.NCC, 
         trm.region.WCVI, trm.region.QCS, trm.region.SOG, 
         file = file.path( model.dir, paste0('trm_allModels_', x, '.RData')) )
   
   save( trm.build.results, file = file.path( model.dir, paste0('trmbuildResults_', x, '.RData')) )
   
}
# End Trimmed Ranger run and save. 
